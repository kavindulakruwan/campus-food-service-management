...
exports.createOrder = async (req, res) => {
  try {
    ...
    const orderItems = Array.isArray(items) && items.length > 0
      ? items.map((item) => ({ name: String(item.name || 'Custom Meal'), quantity: Math.max(1, Number(item.quantity) || 1), price: Math.max(0, Number(item.price) || 0) })).filter((item) => item.name && item.quantity > 0)
      : [];
...
exports.cancelOrder = async (req, res) => {
  try {
    ...
    if (!order) return res.status(404).json({ success: false, message: 'Order not found' });
    if (order.user.toString() !== req.user.id && req.user.role !== 'admin') return res.status(403).json({ success: false, message: 'Not authorized' });
    if (!['Pending', 'Processing'].includes(order.status)) return res.status(400).json({ success: false, message: 'Cannot cancel order in current status' });
