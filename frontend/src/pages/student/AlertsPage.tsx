...
const AlertsPage = () => {
  ...
  const loadAlerts = () => {
    const loadedNotices = alertsStore.getPublishedNoticesForRole(role)
    const loadedNotifications = alertsStore.getNotificationsForRole(role)
  ...
  useEffect(() => {
    ...
    const handleExternalUpdate = (event: StorageEvent | Event) => {
      // Always reload when we detect an update
      if (event instanceof StorageEvent) {
        if (event.key === alertsStore.getStorageKey()) {
          loadAlerts()
        }
      } else {
        // alerts-updated event from alertsStore
        loadAlerts()
      }
    }
