...
const STATUS_COLORS: Record<string, string> = {
  ...
  delivered: 'bg-green-200 text-green-900', Completed: 'bg-green-200 text-green-900',
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex gap-2">
          <button onClick={() => setActiveTab('menu')} className={`px-4 py-2 rounded-lg text-sm font-semibold transition ${activeTab === 'menu' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
            Menu {cartCount > 0 && <span className="ml-1 bg-orange-500 text-white rounded-full px-2 py-0.5 text-xs">{cartCount}</span>}
          </button>
          <button onClick={() => setActiveTab('history')} className={`px-4 py-2 rounded-lg text-sm font-semibold transition ${activeTab === 'history' ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
            My Orders
          </button>
        </div>
      </div>
                  ...
                  <div key={item.id} className="rounded-xl border border-gray-100 p-4 flex items-center justify-between gap-4 hover:border-orange-200 transition">
                    ...
                    {inCart ? (
                      <div className="flex items-center gap-2">
                        <button onClick={() => updateQty(item.id, inCart.quantity - 1)} className="w-8 h-8 rounded-full bg-gray-100 font-bold flex items-center justify-center">−</button>
                        <span className="w-6 text-center font-semibold">{inCart.quantity}</span>
                        <button onClick={() => updateQty(item.id, inCart.quantity + 1)} className="w-8 h-8 rounded-full bg-gray-900 text-white font-bold flex items-center justify-center">+</button>
                      </div>
                    ) : (
                      <button onClick={() => addToCart(item)} className="px-4 py-2 rounded-lg bg-orange-600 text-white text-sm font-semibold hover:bg-orange-700">Add</button>
                    )}
                  </div>
                );
              })}
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden">
              <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center">
                ...
                {cart.length > 0 && <button onClick={() => setCart([])} className="text-xs text-red-500">Clear all</button>}
              </div>
              <div className="p-4">
                {cart.length === 0 ? (
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-4 space-y-3">
              ...
              <div>
                ...
                <input type="text" value={deliveryAddress} onChange={e => setDeliveryAddress(e.target.value)} placeholder="e.g. Room 204, Block A" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Special Instructions</label>
                <textarea value={specialInstructions} onChange={e => setSpecialInstructions(e.target.value)} rows={2} className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />
              </div>
              <div>
              ...
              <button onClick={handleCreateOrder} disabled={creating || cart.length === 0} className="w-full py-3 rounded-xl bg-orange-600 text-white font-semibold text-sm hover:bg-orange-700 disabled:opacity-50">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1 flex-wrap">
                          ...
                          {order.paymentStatus === 'Pending' && order.status !== 'Cancelled' && (
                            <button onClick={() => navigate(`/payments?orderId=${order._id}&amount=${order.totalAmount}`)} className="text-xs px-2 py-1 rounded bg-green-50 text-green-700 hover:bg-green-100 font-semibold">Pay</button>
                          )}
                          {['pending', 'confirmed', 'Pending', 'Processing'].includes(order.status) && (
                            <button onClick={() => handleCancel(order._id)} disabled={cancelling === order._id} className="text-xs px-2 py-1 rounded bg-red-50 text-red-600 hover:bg-red-100 disabled:opacity-50">
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-end gap-2 px-6 py-4 border-t border-gray-100">
              {['pending', 'confirmed', 'Pending', 'Processing'].includes(viewOrder.status) && (
                <button onClick={() => handleCancel(viewOrder._id)} className="px-4 py-2 rounded-lg bg-red-50 text-red-600 text-sm font-semibold hover:bg-red-100">Cancel Order</button>
              )}
