...
const emptyForm = {
  ...
  isAvailable: true,
  offer: {
    type: 'none' as 'none' | 'discount' | 'combo',
    title: '',
    discountPercent: 10,
    comboText: '',
    isActive: false,
...
const AdminMealManagementPage = () => {
  ...
  const openEdit = (meal: MealItem) => {
    ...
    setEditForm({
      ...
      offer: {
        type: meal.offer?.type || 'none',
        title: meal.offer?.title || '',
        discountPercent: meal.offer?.discountPercent || 10,
        comboText: meal.offer?.comboText || '',
        isActive: meal.offer?.isActive || false,
      },
    })
  }
        ...
        {createOpen && (
          <form onSubmit={handleCreate} className="mt-4 grid gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2 rounded-2xl border border-amber-200 bg-amber-50 p-4">
              <p className="text-xs font-bold uppercase tracking-wide text-amber-700">Offer / Discount</p>
              <div className="mt-3 grid gap-3 md:grid-cols-2">
                <div>
                  <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-600">Offer Type</label>
                  <select
                    value={createForm.offer.type}
                    onChange={(event) => setCreateForm((prev) => ({
                      ...prev,
                      offer: { ...prev.offer, type: event.target.value as 'none' | 'discount' | 'combo' },
                    }))}
                    className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"
                  >
                ...
                <div>
                  <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-600">Offer Title</label>
                  <input
                    value={createForm.offer.title}
                    onChange={(event) => setCreateForm((prev) => ({ ...prev, offer: { ...prev.offer, title: event.target.value } }))}
                    className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"
                    placeholder="e.g. 10% OFF Lunch Deal"
                  />
                </div>
                {createForm.offer.type === 'discount' && (
                  <div>
                    <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-600">Discount %</label>
                    <input
                      type="number"
                      min={1}
                      max={100}
                      value={createForm.offer.discountPercent}
                      onChange={(event) => setCreateForm((prev) => ({ ...prev, offer: { ...prev.offer, discountPercent: Number(event.target.value) } }))}
                      className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"
                    />
                  </div>
                )}
                {createForm.offer.type === 'combo' && (
                  <div>
                    <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-600">Combo Details</label>
                    <input
                      value={createForm.offer.comboText}
                      onChange={(event) => setCreateForm((prev) => ({ ...prev, offer: { ...prev.offer, comboText: event.target.value } }))}
                      className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"
                ...
                <div className="flex items-center gap-2 md:col-span-2">
                  <input
                    id="create-offer-active"
                    type="checkbox"
                    checked={createForm.offer.isActive}
                    onChange={(event) => setCreateForm((prev) => ({ ...prev, offer: { ...prev.offer, isActive: event.target.checked } }))}
                  />
                  <label htmlFor="create-offer-active" className="text-sm font-medium text-slate-700">Activate this offer</label>
                </div>
              </div>
            </div>
        ...
        {loading ? (
          ...
          <article key={meal.id} className="group relative mx-auto w-full max-w-[265px] overflow-hidden rounded-3xl border border-white/40 bg-white/35 text-slate-900 shadow-[0_14px_40px_-20px_rgba(15,23,42,0.45)] ring-1 ring-white/30 backdrop-blur-xl transition duration-300 hover:-translate-y-1 hover:bg-white/45 hover:shadow-[0_24px_50px_-24px_rgba(15,23,42,0.55)]">
            ...
            <div className="relative h-36 w-full overflow-hidden border-b border-white/30 bg-slate-100/70">
              ...
              <span className={`absolute right-2.5 top-2.5 rounded-full border px-2 py-0.5 text-[10px] font-semibold backdrop-blur-md ${meal.isAvailable ? 'border-emerald-200/60 bg-emerald-100/75 text-emerald-800' : 'border-rose-200/70 bg-rose-100/75 text-rose-800'}`}>
                {meal.isAvailable ? 'Available' : 'Out of Stock'}
              </span>
              <span className="absolute left-2.5 top-2.5 rounded-full border border-slate-100/30 bg-slate-900/65 px-2 py-0.5 text-[10px] font-semibold text-white backdrop-blur-md">
                Stock: {meal.quantity}
              </span>
              {meal.offer?.isActive && meal.offer.type !== 'none' && (
                <span className="absolute bottom-2.5 left-2.5 rounded-full border border-orange-100/70 bg-orange-500/85 px-2 py-0.5 text-[10px] font-semibold text-white backdrop-blur-md">
                  {meal.offer.type === 'discount' ? `${meal.offer.discountPercent}% OFF` : meal.offer.title || 'Combo Offer'}
                </span>
              )}
            </div>
            ...
            <div className="relative space-y-2.5 p-3">
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full border border-white/60 bg-white/35 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-600 backdrop-blur-sm">{meal.category}</span>
              </div>
              ...
              {meal.offer?.isActive && meal.offer.type !== 'none' && (
                <div className="rounded-xl border border-amber-100/80 bg-amber-50/65 px-3 py-2 text-xs text-amber-900 backdrop-blur-sm">
                  <p className="font-bold uppercase tracking-wide">{meal.offer.title || 'Special offer'}</p>
                  <p className="mt-1">
                    {meal.offer.type === 'discount'
                      ? `Save ${meal.offer.discountPercent}% on this meal.`
                      : meal.offer.comboText || 'Combo offer available.'}
                  </p>
                </div>
              )}

              <p className="pt-0.5 text-3xl font-black text-orange-500 drop-shadow-[0_2px_6px_rgba(251,146,60,0.35)]">LKR {meal.price.toFixed(0)}</p>

              <div className="grid grid-cols-2 gap-2">
                <button type="button" onClick={() => openEdit(meal)} className="inline-flex items-center justify-center gap-1 rounded-xl border border-slate-200/70 bg-slate-900/90 px-3 py-1.5 text-xs font-semibold text-white shadow-sm backdrop-blur-md transition hover:bg-slate-800 sm:text-sm"><PencilLine className="h-3.5 w-3.5" /> Edit</button>
                <button type="button" onClick={() => handleDelete(meal)} className="inline-flex items-center justify-center gap-1 rounded-xl border border-orange-200/70 bg-orange-500/90 px-3 py-1.5 text-xs font-semibold text-white shadow-sm backdrop-blur-md transition hover:bg-orange-600 sm:text-sm"><Trash2 className="h-3.5 w-3.5" /> Delete</button>
              </div>
            </div>
          </article>
        ))}
              ...
              <div className="md:col-span-2 rounded-2xl border border-amber-200 bg-amber-50 p-4">
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  ...
                  <div>
                    <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-600">Offer Title</label>
                    <input
                      value={editForm.offer.title}
                      onChange={(event) => setEditForm((prev) => ({ ...prev, offer: { ...prev.offer, title: event.target.value } }))}
                      className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"
                      placeholder="e.g. 10% OFF Lunch Deal"
                    />
                  </div>
                  {editForm.offer.type === 'discount' && (
                    <div>
                      <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-600">Discount %</label>
                      <input
                        type="number"
                        min={1}
                        max={100}
                        value={editForm.offer.discountPercent}
                        onChange={(event) => setEditForm((prev) => ({ ...prev, offer: { ...prev.offer, discountPercent: Number(event.target.value) } }))}
                        className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"
                      />
                    </div>
                  )}
                  {editForm.offer.type === 'combo' && (
                    <div>
                      <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-slate-600">Combo Details</label>
                      <input
                        value={editForm.offer.comboText}
                        onChange={(event) => setEditForm((prev) => ({ ...prev, offer: { ...prev.offer, comboText: event.target.value } }))}
                        className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm"
                        placeholder="e.g. Burger + Drink"
                      />
                    </div>
                  ...
                  <div className="flex items-center gap-2 md:col-span-2">
                    <input
                      ...
                      type="checkbox"
                      checked={editForm.offer.isActive}
                      onChange={(event) => setEditForm((prev) => ({ ...prev, offer: { ...prev.offer, isActive: event.target.checked } }))}
                    />
                    <label htmlFor="edit-offer-active" className="text-sm font-medium text-slate-700">Activate this offer</label>
                  </div>
                </div>
              </div>
              <input
